...
public class ShopifyModule extends ReactContextBaseJavaModule {
  ...
  @ReactMethod
  public void initStore(String shopDomain, String apiKey, Promise promise) {
    client =
          ...
          collections
            ...
            .edges(edge ->
              edge
                .cursor()
                .node(collection ->
                  collection.handle().title().description().image(i -> i.url())
                )
            )
      )
    );
            ...
            edge.node(pNode ->
              pNode
                ...
                .images(
                  args -> args.first(250),
                  iEdges ->
                    iEdges.edges(iEdge -> iEdge.node(iNode -> iNode.url()))
                )
                .variants(
                  args -> args.first(250),
                  vEdges ->
                    vEdges.edges(vEdge ->
                      vEdge.node(vNode ->
                        vNode
                          ...
                          .weight()
                          .weightUnit()
                          .selectedOptions(so -> so.name().value())
                      )
                    )
                )
            )
  ...
  public void createCustomer(ReadableMap customerQuery, Promise promise) {
    ...
    Storefront.MutationQuery mutationQuery = Storefront.mutation(mutation ->
      mutation.customerCreate(
        ...
        query ->
          query
            .customer(customer -> customer.id().email().firstName().lastName())
            .customerUserErrors(userError -> userError.field().message())
      )
    );
  ...
  @ReactMethod
  public void createCustomerAddress(ReadableMap addressInfo, Promise promise) {
    final String accessToken = this.getAccessToken();
  ...
  @ReactMethod
  public void updateCustomerDefaultAddress(String addressId, Promise promise) {
    final String accessToken = this.getAccessToken();
                  ...
                  edge
                    ...
                    .node(node ->
                      node
                        ...
                        .financialStatus()
                        .fulfillmentStatus()
                        .totalPriceV2(price -> price.amount().currencyCode())
                        ...
                        .lineItems(
                          args -> args.first(25).after(lineItemsCursor),
                          vEdges ->
                            vEdges
                              .edges(vEdge ->
                                vEdge
        ...
        checkoutCreate ->
          checkoutCreate
            .checkout(checkout ->
              checkout
                .webUrl()
                .paymentDueV2(paymentDue -> paymentDue.amount().currencyCode())
                .requiresShipping()
